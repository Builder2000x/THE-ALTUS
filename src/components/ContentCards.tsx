...
export function ArticleCard({article,featured=false}:{article:Article;featured?:boolean}){return <article className={'article-card '+(featured?'featured':'')}><ArticleDestination article={article} className={'card-image'+(article.href?' newsletter-artwork':'')}><ArticleImage article={article}/></ArticleDestination><div className="card-meta"><span>{article.category}</span><time dateTime={article.publishedAt}>{new Intl.DateTimeFormat('en',{month:'short',day:'numeric',year:'numeric'}).format(new Date(article.publishedAt))}</time></div><h3><ArticleDestination article={article}>{article.title}</ArticleDestination></h3><p>{article.excerpt}</p></article>}
