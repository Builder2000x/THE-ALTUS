...
export async function POST(request: Request) {
  ...
  if (contentLength > 2_048) return NextResponse.json({ error: 'Invalid request.' }, { status: 413 })
  ...
  try {
    const response = await fetch(`https://api.beehiiv.com/v2/publications/${publicationId}/subscriptions`, {
      ...
      body: JSON.stringify({ email: normalizedEmail, send_welcome_email: true, utm_source: 'altusnews.com', utm_medium: 'website' }),
